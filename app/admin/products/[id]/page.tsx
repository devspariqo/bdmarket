...
export default async function EditProductPage({ params }: { params: { id: string } }) {
  ...
  if (params.id === 'new') {
    ...
    return (
      <div className="space-y-5">
        <div>
          <Link href={`${base}/products`} className="mb-3 inline-flex items-center gap-1.5 text-[13px] font-semibold text-brand-700 hover:underline">
            <ArrowLeft className="h-3.5 w-3.5" /> Back to products
          </Link>
          <h1 className="font-display text-2xl font-bold tracking-tight text-ink-900">Add New Product</h1>
        </div>
        <ProductEditor categories={categories} brands={brands} product={null} base={base} />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <ProductEditor categories={categories} brands={brands} product={product} base={base} />
    </div>
  );
}
